...
var shutdownEmitter = new node_events_1.EventEmitter();
shutdownEmitter.once('shutdown', disconnectDatabase);
process.on('SIGINT', function () { return shutdownEmitter.emit('shutdown'); }); // Ctrl + C 終止
process.on('SIGTERM', function () { return shutdownEmitter.emit('shutdown'); }); // 系統發出的終止訊號
process.on('exit', function () { return shutdownEmitter.emit('shutdown'); }); // Node.js 正常退出時執行
